...
### Dashboard A-Frame 3-D view (spajeti_aircraft_3d.html) <span style="color: #f59e0b; font-weight: bold">[PARTIAL — camera fixed v1.19.0]</span>

Separate from `plot_aircraft.py` — generated by `write_spajeti_aircraft_3d_report`
in `run_horizontal_small_uav.py`.  Output: `outputs/.../reports/spajeti_aircraft_3d.html`.

- **Camera was too zoomed-in** — original camera at `z=4.0` showed only a partial
  wing.  Fixed in v1.19.0: camera moved to `z=7.5`, `y=1.8`, pitch `−12°` so the
  full aircraft fits in frame.
- **[TODO] Verify framing** — confirm after the next successful run that wing,
  all four VTP panels, fuselage mesh, and engine nozzle are all visible in the
...
Goal: keep full control of the H-wing aeroelastic model while improving it toward
a credible pre-NASTRAN flutter/load screen. AVL/VLM may be useful later for
calibration, but the optimizer-facing model should remain internal, smooth, and
inspectable.

### Step 1 - Replace constant lift with a swappable load-distribution backend <span style="color: #22c55e; font-weight: bold">[DONE - v1.20.0]</span>

Current method is `ConstantLiftLoads`, which assumes:

    lift_per_span = total_lift / span

This is only a placeholder. `SchrenkLiftDistribution` is now implemented as the
first swappable backend and kept separate from the optimizer until review.
Backend path:

- `SchrenkLiftDistribution` first: trapezoidal chord loading averaged with
  elliptical loading, normalized to the requested total lift.
- Later `LiftingLineLiftDistribution`: taper, twist, local airfoil data, and
  optional endplate correction.
- Later `AVLLoadDistribution` or surrogate backend: use AVL only as an external
  calibration/source of load shapes, not as a hard dependency in the optimizer.

Outputs needed by structures/flutter:
...
- shear `V(y)`
- bending moment `M(y)`
- torque `T(y)` about the elastic axis
- root bending, root torque, tip deflection, tip twist

### Step 2 - Connect structural `t/c` to aerodynamic wing `t/c` <span style="color: #22c55e; font-weight: bold">[DONE - v1.21.0]</span>

Previous inconsistency:

- `aircraft:wing:thickness_to_chord` fed the structural wingbox.
- `wing_section_tc` feeds airfoil/M_crit logic.

Implemented fix:

1. Added `aeroelasticity:structural_thickness_to_chord`.
2. Connected `wing_section_tc` to `aeroelasticity:structural_thickness_to_chord`
   in `run_horizontal_small_uav.py`.
3. `WingboxStructuralEstimate` now uses this aeroelasticity input instead of the
   fixed CSV `aircraft:wing:thickness_to_chord`.

Validation completed:

1. Full-model sensitivity check confirmed `aeroelasticity:structural_thickness_to_chord`
   follows `wing_section_tc` at `t/c = 0.05, 0.15, 0.18`.
2. `EI`, `GJ`, and divergence speed increase monotonically with `wing_section_tc`.
3. The thin case (`t/c = 0.05`) correctly becomes much less stable in the flutter
   screen, while thicker cases recover negative design-point eigenvalue.
4. Added a regression test for monotonic `EI/GJ` response to structural `t/c`.

Keep the actual box height relation explicit:

       box_height = box_height_fraction * wing_section_tc * chord

### Step 3 - Spanwise beam structural model <span style="color: #22c55e; font-weight: bold">[DONE - v1.25.0]</span>

Replace the single equivalent wingbox with spanwise stations:

- local chord, local `t/c`, local box width/height
- local `EI(y)`, `GJ(y)`, mass/span, pitch inertia/span
- distributed VTP, fuel, engine, servo, and elevon masses where appropriate

Implemented pieces:

- `SpanwiseWingboxProperties` computes half-wing station arrays for `y`, chord,
  box width/height, `EI(y)`, `GJ(y)`, mass/span, and pitch inertia/span.
- The component is wired into `AeroelasticityGroup` as analysis-only with unique
  output names; existing divergence/flutter constraints still use the old scalar
  equivalent wingbox until beam integration is ready.
- Tests confirm tapered wings have larger root stiffness/mass than tip values,
  and all station stiffnesses increase with structural `t/c`.
- `SchrenkLiftDistribution` is now wired into `AeroelasticityGroup` for its
  distributed load arrays only; its legacy scalar summary outputs are not
  promoted, avoiding collisions with the active constant-load checks.
- `SpanwiseBeamResponse` integrates Schrenk `M(y)` and `T(y)` against local
  `EI(y)` and `GJ(y)` to report spanwise slope, deflection, twist, and
  Schrenk-specific tip deflection/twist.
- The new beam response is reporting-only for now; it does not yet replace the
  scalar divergence/flutter/strength constraints.
- `SpanwiseMassDistribution` combines structural wingbox mass with explicit VTP
  tip, engine, fuel, servo, and elevon mass terms. It reports added mass/span,
  total mass/span, total pitch inertia/span, and total half-wing mass for the
  future beam modal model.
- The spanwise mass model uses explicit aeroelastic mass inputs, avoiding the
  current `aircraft:vertical_tail:mass` zero-mass issue from the H-wing FLOPS
  mass-scaler setup.
- `SpanwiseEquivalentProperties` reduces spanwise `EI(y)`, `GJ(y)`, total
  mass/span, and pitch inertia/span into scalar properties for the current
  divergence, flutter, P-K, load, and strength checks.
- Active scalar aeroelastic constraints now consume the spanwise-equivalent
  properties. `WingboxStructuralEstimate` remains only for root geometry,
  elastic-axis, and control bookkeeping.
- Live engine mass and mission fuel mass are connected into the spanwise mass
  model. VTP tip, servo, and elevon masses use explicit preliminary aeroelastic
  defaults until a full component weight model exists.

Validation:

- unit tests cover Schrenk loads, spanwise wingbox properties, beam response,
  mass distribution, and spanwise-equivalent scalar reduction.
- full model smoke checks confirmed the spanwise-equivalent outputs are live.
- optimization validation completed for the v1.25.x model. IPOPT reported local
  infeasibility, driven by small trajectory equality residuals, but the final
  physical model-level constraint table passed: Ny, Nz, M_crit margin,
  divergence margin, design-point flutter eigenvalue, SLS thrust, fuel budget,
  and engine mass.

### Step 4 - Beam modal flutter model <span style="color: #22c55e; font-weight: bold">[DONE - v1.27.0]</span>

Upgrade from the current 3-DOF lumped model to a beam modal model:

Implemented:

- `BeamModalFlutter` computes one finite-element bending mode and one
  finite-element torsion mode from the spanwise Step-3 arrays.
- It projects quasi-steady strip-theory lift/moment terms into modal
  coordinates.
- It reports dry bending/torsion frequencies, design-point modal max-real
  eigenvalue, modal flutter speed, modal flutter margin, and critical mode.
- It now also runs a reduced-frequency Theodorsen P-K iteration on the beam
  modes and reports modal P-K flutter speed, frequency, speed margin, mode
  damping, mode frequency, and convergence status.
- The old scalar `lambda_max` constraint remains active until this modal screen
  is calibrated and checked over a wider design set.

Calibration decisions intentionally deferred to Step 5:

- decide when to make `beam_modal_max_real_eigenvalue_at_design <= 0` or a
  beam-modal P-K margin the active optimizer constraint,
- add H-wing VTP/endplate aerodynamic strip contributions, not only mass/inertia,
- replace Theodorsen strip theory with calibrated generalized aerodynamic
  forces when a VLM/DLM/AVL-derived load source is available.
...
- DONE (v1.29.0): extend `BeamModalFlutter` from 2-mode [bending, torsion] to
  ...
  - Control mode shape φ_δ(y) is a rigid binary rotation (1 over elevon span, 0 elsewhere).
  - 3×3 modal mass matrix includes bending–control and torsion–control cross terms
    from `CONTROL_STATIC_UNBALANCE` and `CONTROL_INERTIA_PER_UNIT_SPAN`.
  - 3×3 GAF: [h, α] block uses Theodorsen C(k); δ column is quasi-steady
    (Theodorsen-Garrick T-functions deferred). δ row uses same C(k) scaling as
    cl_alpha/cm_alpha for the ch_alpha terms (consistent with `PKFlutterAnalysis`).
  - `CONTROL_STIFFNESS` feeds the modal hinge spring K_δ directly.
...
- DONE (v1.29.0): switch the active optimizer flutter constraint from the old
  quasi-steady `MAX_REAL_EIGENVALUE_AT_DESIGN <= 0` (2-DOF lumped) to the beam-modal
