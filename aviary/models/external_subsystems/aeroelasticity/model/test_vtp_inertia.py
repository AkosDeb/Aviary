...
class TestVTPTipInertia(unittest.TestCase):
    def make_problem(self):
        ...
        prob.set_val(f'vtp.{AE.VTP_AREAL_DENSITY}', 1.25, units='kg/m**2')
        prob.set_val(f'vtp.{AE.VTP_TIP_PANEL_COUNT}', 2.0)
        return prob
    ...
    def test_symmetric_vtp_adds_mass_inertia_and_static_unbalance(self):
        ...
        self.assertGreater(float(tip_inertia[0]), 0.0)
    ...
    def test_more_vtp_areal_density_increases_equivalent_mass_and_inertia(self):
        prob = self.make_problem()
        prob.set_val(f'vtp.{AE.VTP_AREAL_DENSITY}', 0.75, units='kg/m**2')
        prob.run_model()
        low_mass = float(
        ...
        prob.set_val(f'vtp.{AE.VTP_AREAL_DENSITY}', 1.50, units='kg/m**2')
        prob.run_model()
        high_mass = float(
            prob.get_val(f'vtp.{AE.VTP_EQUIVALENT_MASS_PER_UNIT_SPAN}', units='kg/m')[0]
        )
    ...
    def test_more_vtp_span_increases_tip_mass_and_inertia(self):
        prob = self.make_problem()
        prob.set_val(f'vtp.{Aircraft.VerticalTail.SPAN}', 0.25, units='m')
        prob.run_model()
        ...
        prob.set_val(f'vtp.{Aircraft.VerticalTail.SPAN}', 0.50, units='m')
        prob.run_model()
