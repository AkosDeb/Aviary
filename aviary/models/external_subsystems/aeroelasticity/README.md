...
| `VTPTipInertia` | `model/vtp_inertia.py` | Compatibility adapter from tail-geometry-owned mass/inertia equivalents to the legacy VTP tip mass, pitch inertia, and chordwise offset outputs. |
...
- Concentrated masses are smeared to nearby spanwise stations for optimizer
  robustness.
- VTP tip mass uses the existing half-wing convention, but the mass and inertia
  equivalents are now generated by the active tail geometry backend and mapped
  through `VTPTipInertia`.
